...
<div class="container-contact100">
		<div class="wrap-contact100">   
                        <form action = "{{ route('RisquePort.store')}}" method = "post">
                    @csrf
                    <span class="contact100-form-title">Formulaire d'analyse du Risque sur un port</span>
                        <fieldset>
                                    
                                <p class = "analyse">
                                <center><h3> Localisation</h3></center>
                                        <div class = "form-group">
                                            <label for = "port_id">Port, Carte, Module, Equipement, Division, Departement, Direction</label>
                                            <select name="port_id" id="port" class="form-control">
                                                @foreach($ports as $port)
                                                    <option value="{{ $port->id }}">{{$port->id}}-{{$port->nom_port}}, {{$port->carte}}, {{$port->module}}, 
                                                    {{$port->equipement}}, {{$port->division}}, {{$port->departement}}, {{$port->departement}}, 
                                    ...
                                    <p>
                                        ...
                                        <div class = "form-group">
                                            ...
                                            <select name="titre" id="titre" class="form-control">
                                                @foreach($ports as $port)
                                                    <option value="{{ $port->nom_port }}">{{$port->id}}-{{$port->nom_port}}-{{$port->type_port}} </option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class = "form-group">
